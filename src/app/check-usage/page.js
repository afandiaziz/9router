...
function CopyIcon({copied}) {
	return (
		<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={copied ? "#16a34a" : "currentColor"} strokeWidth="2" className="opacity-70 shrink-0">
...
function CopyButton({text, label}) {
	...
	return (
		<button type="button" onClick={copy} className="b-chip hover-tilt-shake" title={`Copy ${label || text}`}>
			{copied ? "✓ Copied" : "Copy"}
...
function Chip({text}) {
	...
	return (
		<span onClick={copy} title={`Click to copy: ${text}`} className={`b-chip select-all ${copied ? "b-chip-pop" : ""}`}>
			{text}
			<CopyIcon copied={copied} />
		</span>
	);
		...
		<div className="b-model-card">
			<div className="b-model-head">
				<div className="b-model-name-wrapper" onClick={copy} title={`Click to copy: ${modelText}`}>
					<span ref={nameRef} className={`b-model-name ${copied ? "b-chip-pop" : ""}`} style={{"--fit-size": `${fitSize}px`}}>
						{modelText}
					</span>
				</div>
				<button type="button" onClick={copy} className="b-copy-btn" title={`Copy ${modelText}`}>
					{copied ? "✓ Copied" : "Copy"}
					<CopyIcon copied={copied} />
				</button>
			</div>
...
export default function CheckUsagePage() {
	...
	const barColor = result?.percent > 90 ? "hsl(0 84% 55%)" : result?.percent > 70 ? "hsl(var(--brutal-yellow))" : "hsl(var(--primary))";
				...
				{!result ? (
					<form onSubmit={handleSubmit} className="b-card shadow-brutal p-6 space-y-4 max-w-xl mx-auto">
						<div className="relative">
							<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--muted-foreground))" strokeWidth="2" className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
								<circle cx="7.5" cy="15.5" r="5.5" />
								<path d="M21 2l-9.6 9.6" />
								<path d="M15.5 7.5l3 3L22 7l-3-3" />
							</svg>
							<input type="text" value={key} onChange={(e) => setKey(e.target.value)} placeholder="sk-danton-xxxxxxxxxxxxxxxx" className="b-input" style={{paddingLeft: "2.5rem"}} disabled={loading} />
						</div>
					...
					<div className="space-y-4 animate-reveal-up">
						<div className="flex flex-col gap-6 items-start">
							...
							<div className="space-y-4 w-full">
								...
								<div className="flex gap-3 w-full">
									<button onClick={handleRefresh} disabled={refreshing} className="b-btn flex-1">
										<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={refreshing ? "animate-spin-slow" : ""}>
											<path d="M23 4v6h-6" />
											<path d="M1 20v-6h6" />
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
										...
										<div className="space-y-5">
											...
											<div className="flex justify-between items-center gap-3">
												...
												<span className={`b-badge ${result.isActive ? "b-badge-ok" : "b-badge-bad"}`}>{result.isActive ? "Active" : "Disabled"}</span>
											</div>
											...
											<div>
												...
												<div className="b-progress-track">
													<div className="b-progress-fill" style={{width: `${barWidth}%`, background: barColor, borderRight: barWidth ? "2px solid #000" : "none"}} />
												</div>
							...
							<div className="b-card shadow-brutal p-6 space-y-5 w-full">
								{/* Header */}
								<div className="flex justify-between items-center gap-3">
									<div className="min-w-0">
									...
									<div className="flex items-center gap-2">
										<button type="button" onClick={fetchAllTimeStats} disabled={allTimeLoading} className="b-chip hover-tilt-shake" title="Refresh All Time Stats">
											<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={allTimeLoading ? "animate-spin-slow" : ""}>
												<path d="M23 4v6h-6" />
												<path d="M1 20v-6h6" />
												<path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />
											</svg>
